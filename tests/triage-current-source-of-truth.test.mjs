...
test("contador, fila, executor, histórico e análises usam a mesma triagem vigente", async () => {
  ...
  assert.match(helper, /length\(trim\(coalesce\(.*description.*\)\)\) >= 80/s);
  ...
  assert.match(schema, /triageInputUpdatedAt: integer\("triage_input_updated_at"/);
  assert.match(migration, /profile_revision.*rules_revision.*instructions_revision.*created_at/s);
  assert.match(migration, /ALTER TABLE `jobs` ADD `triage_input_updated_at`/);
  assert.match(migration, /UPDATE `draft_outbox`/);
  assert.match(migration, /SET `auto_send_authorized` = false/);
  assert.match(migration, /WHERE `status` IN \('pending', 'checking', 'drafted', 'failed'\)/);
});

test("origens automáticas criam rascunho sem autorizar envio", async () => {
  const [run, aiVerdicts, csvImport, jobAnalysis, outbox, queue, screen] = await Promise.all([
    read("../app/api/triage/run/route.ts"),
  ...
  assert.match(run, /autoSendAuthorized: false, autoSendAuthorizedAt: null/);
  for (const source of [aiVerdicts, csvImport, jobAnalysis]) assert.match(source, /queueApprovedDraftOutbox/);
