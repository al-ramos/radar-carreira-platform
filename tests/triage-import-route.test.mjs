...
test("reimportação de análise externa substitui veredito, gera rascunho quando seguro e é restrita ao proprietário", async () => {
  const [route, outbox] = await Promise.all([
    readFile(new URL("../app/api/admin/triage-import/route.ts", import.meta.url), "utf8"),
  ...
  assert.match(route, /queueApprovedDraftOutbox/, "veredito aprovado pode entrar na fila de rascunho, como um veredito normal");
  assert.match(outbox, /db\.insert\(draftOutbox\)/);
