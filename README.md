...
Quando a opção **Criar rascunho de verdade no Gmail** estiver ativa, o Radar cria imediatamente o rascunho de uma vaga aprovada (✅), sem bloqueador e com e-mail de contato válido. Isso nunca envia e-mails. Se o conector imediato estiver indisponível, o Worker tenta novamente a fila automaticamente em até 5 minutos e mantém o motivo da última falha visível na vaga. Como contingência adicional, execute `instalarRascunhosAutomaticosRadar` uma única vez no Apps Script: ela cria um acionador a cada 30 minutos para materializar somente os itens já aprovados e pendentes. Esse acionador também respeita o interruptor: desligue-o em **Parâmetros operacionais** para manter a fila pendente até uma ação manual no Portal.

Para atualizar automaticamente os envios manuais, execute `instalarVerificacaoEnviosRadar` **uma única vez** no Apps Script depois de salvar a versão atual do arquivo. Ela instala um gatilho a cada 15 minutos que consulta somente a pasta **Enviados**, registra a última verificação (enviado ou ainda não enviado) e marca no Radar os envios comprovados. Antes de criar um rascunho, o conector repete a mesma conferência por destinatário e assunto exatos; se já houver envio, bloqueia o rascunho e registra a tentativa. A rotina nunca envia e-mails. Para desligá-la, execute `removerVerificacaoEnviosRadar`.
