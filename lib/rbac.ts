 ...
 *
 * SERVER-ONLY: este módulo importa `db/index.ts` (que por sua vez importa
 * `cloudflare:workers`). Nunca importe este arquivo de um componente client
 * ("use client") — quebra o build do bundle do navegador. Componentes
 * client que só precisam de `isOwnerEmail`/`OWNER_EMAIL` devem importar de
 * `./access`, que não tem nenhuma dependência de banco.
 */
