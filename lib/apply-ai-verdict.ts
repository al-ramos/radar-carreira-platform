 ...
 * uma análise por IA considerada válida, ✅ e 🟡 já liberam a fila de
 * rascunho automaticamente, sem exigir reprocessamento manual pelas regras.
 * 🔴/❌ apenas atualizam o veredito, sem enfileirar rascunho.
 */
export async function applyAiVerdicts(userId: string, batchScope: string, entries: AiVerdictEntry[]): Promise<{ applied: number; draftsQueued: number }> {
  if (!entries.length) return { applied: 0, draftsQueued: 0 };
  const db = getDb();
  const profile = await db.select().from(profiles).where(eq(profiles.userId, userId)).limit(1).then((r) => r[0]);
  if (!profile) return { applied: 0, draftsQueued: 0 };
  ...
  for (const entry of entries) {
    ...
    if (entry.verdict === "✅" || entry.verdict === "🟡") {
      ...
      if (isSafeForDraft({ verdict: entry.verdict, contactEmail: job.contactEmail, sourceId: job.sourceId, blocker, deterministicVerdict: deterministic.verdict, deterministicBlocker: deterministic.blocker })) {
        await db.insert(draftOutbox).values({ id: randomUUID(), userId, jobId: job.id, historyId, status: "pending", createdAt: now, updatedAt: now }).onConflictDoNothing();
        draftsQueued += 1;
      }
    }
  }
  await db.update(triageBatches).set({ status: "completed", completedAt: new Date() }).where(eq(triageBatches.id, batchId));
  return { applied, draftsQueued };
}
