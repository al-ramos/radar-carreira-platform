...
export type NormalizedTriageRunRequest = Required<Omit<TriageRunRequest, "referenceDate">> & { referenceDate: string };
...
export function normalizeTriageRunRequest(request: TriageRunRequest, now = new Date()): NormalizedTriageRunRequest {
  ...
  return { trigger: request.trigger, referenceDate, batchSize, reprocess: request.reprocess ?? false, aiMode: request.aiMode ?? "ambiguous", createDrafts: request.createDrafts ?? false };
}
