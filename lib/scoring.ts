...
export type ScoreProfile={masteredSkills:string[];desiredAreas:string[];avoidTerms:string[];seniority?:string|null;preferredMode?:string|null;cities:string[]};
...
export function scoreJob(job:ScoreInput,profile:ScoreProfile){
 ...
 if(profile.seniority&&job.seniority?.toLowerCase().includes(profile.seniority.toLowerCase())){score+=15;reasons.push("Senioridade ideal")}
 if((profile.preferredMode&&job.workMode===profile.preferredMode)||(job.location&&has(job.location.toLowerCase(),profile.cities))){score+=15;reasons.push("Local/modalidade preferida")}
