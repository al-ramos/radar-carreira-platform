...
export type ScoreProfile={masteredSkills:string[];desiredAreas:string[];avoidTerms:string[];seniority:string[];preferredMode:string[];cities:string[]};
...
export function scoreJob(job:ScoreInput,profile:ScoreProfile){
 ...
 const preferredCity=job.location&&profile.cities.some(city=>job.location?.toLowerCase().includes(city.split(",")[0].trim().toLowerCase()));
 if((job.workMode&&profile.preferredMode.some(mode=>job.workMode===mode))||preferredCity){score+=15;reasons.push("Local/modalidade preferida")}
