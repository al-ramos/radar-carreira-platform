...
export default function TriageReport({ close, openJobInRadar, sourceId, sourceLabel, sourceOptions = [], areaOptions = [], channelOptions = [], initialArea = "all", initialChannel = "all", homePeriod = "24" }: { close: () => void; openJobInRadar: (job: Pick<HistoryItem, "jobId" | "externalId" | "jobSource">) => void; sourceId?: string; sourceLabel?: string; sourceOptions?: FilterOption[]; areaOptions?: FilterOption[]; channelOptions?: FilterOption[]; initialArea?: string; initialChannel?: string; homePeriod?: "24" | "72" | "168" | "all" }) {
  ...
  const prepareCodexReview = async (jobIds = aiTargetJobIds) => {
    ...
    try {
      ...
      if (jobIds?.length === 1) setCodexJobStatus("ready");
      setAiPromptOpen(false);
        ...
        {(
          <section id="triage-history" className="triage-history" aria-label="Histórico persistido da nova triagem">
            ...
            <div className="triage-list">
              ...
              <div className="triage-table-wrap"><table className="triage-table"><thead><tr><th className="triage-select-column"><input aria-label="Selecionar todas as vagas visíveis" type="checkbox" checked={allVisibleSelected} onChange={toggleVisibleHistory} /></th><th><button onClick={() => sortHistory("verdict")}>Veredito</button></th><th><button onClick={() => sortHistory("title")}>Vaga</button></th><th><button onClick={() => sortHistory("company")}>Empresa</button></th><th>Fonte / código</th><th>Local e modalidade</th><th>Publicada / recebida</th><th>Contato</th><th><button onClick={() => sortHistory("draft")}>Rascunho</button></th><th>Envio</th><th><button onClick={() => sortHistory("processedAt")}>Analisada</button></th><th>Ações</th></tr></thead><tbody>{visibleHistory.length ? visibleHistory.map((item) => { const draftBlocker = draftActionBlocker(item), isActiveAi = activeAiJobId === item.jobId, isActiveCodex = activeCodexJobId === item.jobId, aiLabel = isActiveAi ? aiReview?.status === "completed" ? "IA concluída" : aiReview?.status === "failed" || aiReview?.status === "partial_failed" ? "IA com falha" : `IA: ${aiReview?.completed ?? 0}/${aiReview?.total ?? 1}` : "Consultar IA", codexLabel = isActiveCodex ? codexJobStatus === "ready" ? "Codex pronto" : codexJobStatus === "failed" ? "Codex falhou" : "Codex: preparando" : "Codex"; return <tr key={item.id} className={rowClass[item.verdict] ?? "backlog"}><td className="triage-select-column"><input aria-label={`Selecionar ${item.title}`} type="checkbox" checked={selectedHistoryJobIds.includes(item.jobId)} onChange={() => toggleHistoryJob(item.jobId)} /></td><td className="triage-verdict">{item.verdict}<small>{item.source === "ai" ? "IA" : item.source === "legacy" ? "Radar" : "Regras"}</small></td><td><button type="button" className="triage-job-link" onClick={() => openJobInRadar(item)} title="Abrir esta vaga no Radar, filtrada pelo código e pela fonte."><b>{item.title}</b></button><span>{item.label}{item.blocker ? ` · ${item.blocker}` : ""}</span>{item.source === "ai" && <details><summary>Evidências</summary><pre>{item.rows}</pre></details>}</td><td>{item.company}</td><td>{item.jobSource ? sourceName(item.jobSource) : "Não informada"}<small>{item.externalId ? `Código ${item.externalId}` : "Sem código"}</small></td><td>{item.workMode ?? "—"}<small>{item.location ?? "Local não informado"}</small></td><td>{item.sourcePublishedAt ? date(item.sourcePublishedAt) : "Não informada"}<small>Recebida: {date(item.receivedAt)}</small></td><td>{item.hasValidContactEmail ? item.contactEmail : "Manual / sem e-mail"}</td><td>{item.draftStatus === "sent" ? "Rascunho usado" : item.draftStatus === "drafted" ? "Pronto" : item.draftStatus === "pending" ? "Na fila" : item.draftStatus === "failed" ? "Falhou" : "—"}</td><td>{item.sentAt ? <><b>Enviado</b><small>{date(item.sentAt)}</small></> : item.draftStatus === "drafted" ? "Ainda não enviado" : "—"}</td><td>{date(item.processedAt)}</td><td><div className="triage-row-actions"><button type="button" onClick={() => void requestAiReview([item.jobId])} disabled={aiReviewLoading || codexQueueLoading || isActiveAi && !["completed", "failed", "partial_failed"].includes(aiReview?.status ?? "")} title={isActiveAi ? "O andamento aparece neste botão; o resultado completo fica no painel de análise acima." : "Analisa esta vaga agora no portal, com a instrução padrão e sem alterar a triagem."}>{aiLabel}</button>{isActiveAi && aiReview?.response && <button type="button" onClick={() => document.querySelector(".triage-ai-review")?.scrollIntoView({ behavior: "smooth", block: "center" })}>Ver resultado</button>}<button type="button" onClick={() => void prepareCodexReview([item.jobId])} disabled={aiReviewLoading || codexQueueLoading || isActiveCodex && codexJobStatus === "preparing"} title={isActiveCodex ? "Quando estiver pronto, peça nesta conversa: analise a última triagem preparada para o Codex." : "Prepara esta vaga para análise nesta conversa do Codex, sem abrir a tela de configuração."}>{codexLabel}</button><button type="button" onClick={() => void queueDrafts([item.jobId])} disabled={queueingDrafts || Boolean(draftBlocker)} title={draftBlocker ?? "Revalida a vaga e prepara somente a fila; não envia e-mail."}>Rascunho</button>{isActiveCodex && codexJobStatus === "ready" && <small>Peça no Codex: analise a última triagem preparada.</small>}{draftBlocker && <small>{draftBlocker}</small>}</div></td></tr>; }) : <tr><td className="triage-table-empty" colSpan={12}>Nenhuma vaga corresponde aos filtros selecionados.</td></tr>}</tbody></table></div>
            </div>
          </section>
        )}
