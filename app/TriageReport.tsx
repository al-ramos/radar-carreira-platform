...
type BatchProgress = { batch: Batch; counts: { queued: number; processing: number; completed: number; failed: number; skipped: number }; recoverable: number; polledAt: string };
...
type LegacyItem = { jobId: string; veredito: string; motivo: string | null; processedAt: string; title: string; company: string; externalId: string | null; sourceId: string | null; sourceName: string | null; workMode: string | null; location: string | null; sourcePublishedAt: string | null; receivedAt: string; url: string; contactEmail: string | null };
...
const isQueueQuotaError = (error: string | null) => Boolean(error && (/daily (?:write )?operations limit/i.test(error) || /\b10253\b/.test(error)));
...
export default function TriageReport({ open = true, close, openJobInRadar, sourceId, sourceLabel, sourceOptions = [], areaOptions = [], channelOptions = [], initialArea = "all", initialChannel = "all", homePeriod = "24", highlightBatchId }: { open?: boolean; close: () => void; openJobInRadar: (job: Pick<HistoryItem, "jobId" | "externalId" | "jobSource">) => void; sourceId?: string; sourceLabel?: string; sourceOptions?: FilterOption[]; areaOptions?: FilterOption[]; channelOptions?: FilterOption[]; initialArea?: string; initialChannel?: string; homePeriod?: "24" | "72" | "168" | "all"; highlightBatchId?: string }) {
  const [message, setMessage] = useState("Carregando avaliações…"),
    ...
    [disqualifyingSelection, setDisqualifyingSelection] = useState(false),
    [openingApplications, setOpeningApplications] = useState(false),
    ...
    [actionCandidate, setActionCandidate] = useState<{ key: string; count: number; total: number; triaged: number; missingDescription: number } | null>(null),
    ...
    [manualProgress, setManualProgress] = useState<BatchProgress | null>(null),
    ...
    [externalAiFilter, setExternalAiFilter] = useState<"all" | "yes" | "no">("all"),
    ...
    [sortKey, setSortKey] = useState<"processedAt" | "company" | "title" | "verdict" | "draft" | "sourceCode" | "locationMode" | "publishedReceived" | "contact" | "externalAi" | "sent">("processedAt"),
    ...
    [csvImportResult, setCsvImportResult] = useState<{ applied: number; draftsQueued: number; draftsCreated: number; emailsSent: number; gmailReason: string | null; notFound: string[]; ambiguous: string[]; rejected: Array<{ line: number; reason: string }> } | null>(null),
  ...
  const loadQueueUsage = async (compact = false) => {
    try {
      const response = await fetch(`/api/triage/queue-usage${compact ? "?compact=1" : ""}`, { cache: "no-store" });
      if (response.ok) {
        const data = await readJsonResponse<Partial<QueueUsage>>(response, "O uso das filas");
        setQueueUsage((current) => current ? { ...current, ...data } : data as QueueUsage);
      }
    } catch { /* A triagem permanece utilizável se a telemetria estiver indisponível. */ }
  };
  const loadManualProgress = async (batchId: string) => {
    try {
      const response = await fetch(`/api/triage/progress?batchId=${encodeURIComponent(batchId)}`, { cache: "no-store" });
      if (!response.ok) return;
      const progress = await readJsonResponse<BatchProgress>(response, "O progresso da triagem");
      setManualProgress(progress);
      setBatches((current) => current.map((batch) => batch.id === progress.batch.id ? { ...batch, ...progress.batch } : batch));
      setLastSyncedAt(new Date(progress.polledAt));
      if (!["queued", "running"].includes(progress.batch.status)) {
        setManualProgress(null);
        void loadHistory();
      }
    } catch { /* Mantém o último progresso visível e tenta novamente no próximo intervalo. */ }
  };
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      if (Date.now() - historyLoadedAtRef.current > 5 * 60_000) void loadHistory();
      void loadCodexQueue(); void loadQueueUsage();
    }, 0);
    return () => window.clearTimeout(timer);
  }, [open]);
  useEffect(() => {
    if (!open) return;
    const timer = window.setInterval(() => { if (document.visibilityState === "visible") void loadQueueUsage(true); }, 120_000);
    return () => window.clearInterval(timer);
  }, [open]);
  useEffect(() => {
    // O catálogo é a fonte de verdade para os nomes. Assim, uma resposta
    // legada de histórico ou da Home que contenha apenas sourceId nunca
    // expõe o UUID no seletor para quem está usando a triagem.
    if (!open) return;
  ...
  useEffect(() => {
    ...
    void fetch(`/api/triage/preview?${query}`, { signal: controller.signal })
      .then(async (response) => response.ok ? response.json() as Promise<{ count?: number; total?: number; triaged?: number; missingDescription?: number }> : Promise.reject(new Error("Falha ao consultar as vagas do recorte da Home.")))
      .then((data) => setActionCandidate({ key: actionSelectionKey, count: typeof data.count === "number" ? data.count : 0, total: typeof data.total === "number" ? data.total : 0, triaged: typeof data.triaged === "number" ? data.triaged : 0, missingDescription: typeof data.missingDescription === "number" ? data.missingDescription : 0 }))
      .catch((error: unknown) => { if (!(error instanceof DOMException && error.name === "AbortError")) setActionCandidate({ key: actionSelectionKey, count: 0, total: 0, triaged: 0, missingDescription: 0 }); });
    return () => controller.abort();
  ...
  const cachedManualItemCounts = latestManualItems.reduce((counts, item) => ({ ...counts, [item.status]: counts[item.status] + 1 }), { queued: 0, processing: 0, completed: 0, failed: 0, skipped: 0 });
  const hasCurrentManualProgress = Boolean(manualProgress && manualProgress.batch.id === latestManual?.id);
  const manualItemCounts = hasCurrentManualProgress && manualProgress ? manualProgress.counts : cachedManualItemCounts;
  const cachedRecoverableManualItemCount = latestManualItems.filter((item) => {
  ...
  const recoverableManualItemCount = hasCurrentManualProgress && manualProgress ? manualProgress.recoverable : cachedRecoverableManualItemCount;
  const queueWindowStartedAt = queueUsage ? new Date(queueUsage.resetAt).getTime() - 24 * 60 * 60_000 : null;
  const manualQuotaRecovered = Boolean(
    latestManual?.status === "failed"
      && isQueueQuotaError(latestManual.error)
      && queueWindowStartedAt
      && new Date(latestManual.completedAt ?? latestManual.createdAt).getTime() < queueWindowStartedAt,
  );
  useEffect(() => {
    if (!manualIsActive || !latestManual?.id) return;
    void loadManualProgress(latestManual.id);
    const timer = window.setInterval(() => { if (document.visibilityState === "visible") void loadManualProgress(latestManual.id); }, 30_000);
    return () => window.clearInterval(timer);
  ...
  const matchesOutreachFilter = (item: HistoryItem) => {
    ...
    if (outreachFilter === "draft_pending") return item.draftStatus === "pending" || item.draftStatus === "checking";
  ...
  const draftCounts = {
    pending: scopedHistory.filter((item) => item.draftStatus === "pending" || item.draftStatus === "checking").length,
  ...
  const orderedHistory = [...filteredHistory].sort((a, b) => {
    const fields = { processedAt: [a.processedAt, b.processedAt], company: [a.company, b.company], title: [a.title, b.title], verdict: [a.verdict, b.verdict], draft: [a.draftStatus ?? "", b.draftStatus ?? ""], sourceCode: [`${sourceLabelFor(a.jobSource, a.jobSourceName)} ${a.externalId ?? ""}`, `${sourceLabelFor(b.jobSource, b.jobSourceName)} ${b.externalId ?? ""}`], locationMode: [`${a.location ?? ""} ${a.workMode ?? ""}`, `${b.location ?? ""} ${b.workMode ?? ""}`], publishedReceived: [`${a.sourcePublishedAt ?? ""} ${a.receivedAt}`, `${b.sourcePublishedAt ?? ""} ${b.receivedAt}`], contact: [a.contactEmail ?? "", b.contactEmail ?? ""], externalAi: [isExternalAiReassessment(a) ? "1" : "0", isExternalAiReassessment(b) ? "1" : "0"], sent: [a.sentAt ?? "", b.sentAt ?? ""] } as const;
  ...
  const draftActionBlocker = (item: HistoryItem) => {
    if (item.draftStatus) return item.draftStatus === "sent" ? (item.gmailSentId ? "Enviado confirmado pelo Gmail" : "Envio informado manualmente") : item.draftStatus === "drafted" ? "Rascunho pronto" : item.draftStatus === "pending" || item.draftStatus === "cancelled" ? null : item.draftStatus === "checking" ? "Consultando a pasta Enviados do Gmail" : item.draftError ?? "A criação imediata falhou; tente novamente após corrigir o conector Gmail.";
  ...
  const draftLabel = (item: HistoryItem) => item.draftStatus === "sent"
    ? item.gmailDraftId ? "Rascunho usado" : "Não houve"
    : item.draftStatus === "drafted" ? "Pronto"
      : item.draftStatus === "pending" ? "Enviando agora"
        : item.draftStatus === "checking" ? "Verificando Gmail"
          : item.draftStatus === "failed" ? "Falhou"
            : item.draftStatus === "cancelled" ? "Não localizado" : "—";
  ...
  const actionSources = sourceId && !periodScopedSourceOptions.some((option) => option.id === sourceId)
    ? [{ id: sourceId, label: sourceLabelFor(sourceId, sourceLabel), count: 0 }, ...periodScopedSourceOptions]
  ...
  const actionMissingDescription = actionCandidate?.key === actionSelectionKey && actionSourceId ? actionCandidate.missingDescription : 0;
  const manualSummary = (batch: Batch) => {
    ...
    if (manualQuotaRecovered) return `O limite da janela anterior já foi renovado. As ${recoverableManualItemCount || batch.total} vaga(s) continuam preservadas e prontas para retomar com sua confirmação.`;
  ...
  const queueDrafts = async (jobIds?: string[]) => {
    ...
    try {
      const requestDraftQueue = () => fetch("/api/triage/drafts/queue", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ action: "queue", sourceId: actionSourceId, roleArea: actionArea, ingestionChannel: actionChannel, homePeriod: actionPeriod, jobIds }),
      });
      let response = await requestDraftQueue();
      let result = await readJsonResponse<{ error?: string; considered: number; queued: number; noValidContact: number; outdated: number; alreadyPresent: number; immediateDraft?: { requested: boolean; created?: number; sent?: number; reconciled?: number; reason?: string } }>(response, "O envio da candidatura");
      ...
      if (jobIds?.length === 1 && result.outdated > 0) {
        ...
        setMessage("A triagem estava desatualizada; reavaliando esta vaga antes do envio…");
        ...
        result = await readJsonResponse<{ error?: string; considered: number; queued: number; noValidContact: number; outdated: number; alreadyPresent: number; immediateDraft?: { requested: boolean; created?: number; sent?: number; reconciled?: number; reason?: string } }>(response, "O envio da candidatura");
        if (!response.ok) throw new Error(result.error ?? "Não foi possível preparar a fila de rascunhos após reavaliar a vaga.");
      }
      const queueMessagePrefix = jobIds?.length ? "Envio processado para a seleção" : "Envio processado para este recorte";
      const immediateMessage = result.immediateDraft?.requested
        ? result.immediateDraft.reconciled ? ` ${result.immediateDraft.reconciled} envio(s) anterior(es) foi(ram) localizado(s) e registrado(s); nenhum novo e-mail foi enviado para essas vagas.` : result.immediateDraft.sent ? ` ${result.immediateDraft.sent} candidatura(s) foi(ram) enviada(s) automaticamente pelo Gmail.` : result.immediateDraft.created ? ` ${result.immediateDraft.created} rascunho(s) foi(ram) criado(s), mas o envio ainda não foi confirmado.` : " O Gmail foi acionado; atualize a tela em instantes para confirmar o envio."
        : ` Envio automático indisponível: ${result.immediateDraft?.reason ?? "tente novamente pela fila."}`;
      ...
      if (jobIds?.length === 1) {
        const status = result.immediateDraft?.requested
          ? result.immediateDraft.reconciled
            ? { kind: "done" as const, text: "Envio anterior localizado no Gmail e registrado; nenhum novo e-mail foi enviado." }
            : result.immediateDraft.sent
            ? { kind: "done" as const, text: "Candidatura enviada automaticamente pelo Gmail." }
            : { kind: "waiting" as const, text: "Gmail acionado; atualize em instantes para confirmar o envio." }
          : { kind: "failed" as const, text: `Envio automático indisponível: ${result.immediateDraft?.reason ?? "tente novamente pela fila."}` };
  ...
  const openSelectedApplications = async (items: HistoryItem[]) => {
    ...
    const tabs = new Map<string, Window>();
    for (const item of candidates) {
      const tab = window.open("about:blank", "_blank");
      if (tab) {
        tab.opener = null;
        tabs.set(item.jobId, tab);
      }
    }
    const jobIds = candidates.filter((item) => tabs.has(item.jobId)).map((item) => item.jobId);
    if (!jobIds.length) {
      setMessage("O navegador bloqueou as abas de candidatura. Permita pop-ups para o Radar e tente novamente.");
      return;
    }
    setOpeningApplications(true);
    setMessage(`Validando e abrindo ${jobIds.length} portal(is) de candidatura…`);
    try {
      const response = await fetch("/api/triage/applications/open", {
        method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ jobIds }),
      });
      const result = await readJsonResponse<{ error?: string; openable: Array<{ jobId: string; destination: string }>; alreadySent: number; unavailable: number; noDestination: number }>(response, "A abertura de candidaturas");
      if (!response.ok) throw new Error(result.error ?? "Não foi possível iniciar as candidaturas.");
      const allowed = new Set(result.openable.map((item) => item.jobId));
      for (const item of result.openable) tabs.get(item.jobId)!.location.href = item.destination;
      for (const [jobId, tab] of tabs) if (!allowed.has(jobId)) tab.close();
      setMessage(`${result.openable.length} portal(is) aberto(s) e marcado(s) como candidatura iniciada. ${result.alreadySent} já enviado(s), ${result.unavailable} indisponível(is) e ${result.noDestination} sem link não foram abertos. O Radar não envia formulários automaticamente.`);
      await loadHistory();
    } catch (error) {
      for (const tab of tabs.values()) tab.close();
      setMessage(error instanceof Error ? error.message : "Não foi possível iniciar as candidaturas.");
    } finally {
      setOpeningApplications(false);
    }
  };
  ...
  const disqualifySelectedJobs = async () => {
    const jobsToDisqualify = selectedHistory.filter((item) => item.verdict !== "❌" && item.verdict !== "🔴");
    if (!jobsToDisqualify.length) {
      setMessage("As vagas selecionadas já estão desclassificadas.");
      return;
    }
    const count = jobsToDisqualify.length;
    if (!window.confirm(`Desclassificar ${count} vaga${count === 1 ? "" : "s"} selecionada${count === 1 ? "" : "s"}? Elas serão marcadas como não aderentes e nenhum rascunho pendente será criado.`)) return;
    setDisqualifyingSelection(true);
    setMessage(`Desclassificando ${count} vaga${count === 1 ? "" : "s"}…`);
    try {
      const response = await fetch("/api/triage/disqualify", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ jobIds: jobsToDisqualify.map((item) => item.jobId) }),
      });
      const result = await readJsonResponse<{ error?: string; count?: number }>(response, "A desclassificação das vagas");
      if (!response.ok) throw new Error(result.error ?? "Não foi possível desclassificar as vagas.");
      setSelectedHistoryJobIds((current) => current.filter((id) => !jobsToDisqualify.some((item) => item.jobId === id)));
      setMessage(`${result.count ?? count} vaga${count === 1 ? "" : "s"} desclassificada${count === 1 ? "" : "s"}. Qualquer rascunho ainda na fila foi cancelado.`);
      await loadHistory();
    } catch (error) {
      setMessage(error instanceof Error ? error.message : "Não foi possível desclassificar as vagas.");
    } finally {
      setDisqualifyingSelection(false);
    }
  };
  const retryFailedDrafts = async () => {
    ...
    try {
      ...
      const result = await response.json() as { error?: string; retried?: number; emailsSent?: number };
      if (!response.ok) throw new Error(result.error ?? "Não foi possível retomar os rascunhos.");
      setMessage(`${result.retried ?? 0} falha(s) retomada(s); ${result.emailsSent ?? 0} candidatura(s) enviada(s) automaticamente.`);
      void loadHistory();
  ...
  const submitCsvImport = async () => {
    ...
    try {
      ...
      const result = await readJsonResponse<{ applied?: number; draftsQueued?: number; draftsCreated?: number; emailsSent?: number; immediateDraft?: { requested?: boolean; reason?: string }; notFound?: string[]; ambiguous?: string[]; rejected?: Array<{ line: number; reason: string }>; error?: string }>(response, "A reimportação da análise");
      if (!response.ok) throw new Error(result.error ?? "Não foi possível reimportar a análise.");
      setCsvImportResult({ applied: result.applied ?? 0, draftsQueued: result.draftsQueued ?? 0, draftsCreated: result.draftsCreated ?? 0, emailsSent: result.emailsSent ?? 0, gmailReason: result.immediateDraft?.requested === false ? result.immediateDraft.reason ?? "Gmail não confirmou o envio" : null, notFound: result.notFound ?? [], ambiguous: result.ambiguous ?? [], rejected: result.rejected ?? [] });
      setMessage(`${result.applied ?? 0} veredito(s) substituído(s)${result.emailsSent ? `, ${result.emailsSent} candidatura(s) enviada(s)` : result.draftsCreated ? `, ${result.draftsCreated} rascunho(s) criado(s), com envio não confirmado` : ""}.`);
      await loadHistory();
    } catch (error) {
      ...
      <section className="modal triage-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <details className="triage-actions">
              ...
              <div className="triage-run-panel">
                ...
                {!(aiPromptOpen && aiTargetJobIds) && <><div className="triage-run-selection" aria-live="polite">
                  {actionCandidateCount === null ? "Selecione uma fonte para consultar o recorte da triagem." : actionCandidateCount === 0 && actionCandidateTotal && actionMissingDescription ? `Nenhuma vaga com descrição íntegra aguarda triagem neste recorte. Há ${actionCandidateTotal} vaga${actionCandidateTotal === 1 ? "" : "s"} no total; ${actionMissingDescription} ${actionMissingDescription === 1 ? "está inconclusiva" : "estão inconclusivas"}.` : actionCandidateCount === 0 && actionCandidateTotal ? `Há ${actionCandidateTotal} vaga${actionCandidateTotal === 1 ? "" : "s"} no recorte ${homePeriodLabel(actionPeriod)}, mas todas já foram triadas.` : actionCandidateCount === 0 ? `Nenhuma vaga corresponde aos filtros da triagem em ${homePeriodLabel(actionPeriod)}.` : `${actionCandidateCount} vaga${actionCandidateCount === 1 ? "" : "s"} aguardando triagem, de ${actionCandidateTotal} no recorte ${homePeriodLabel(actionPeriod)}.`}
                  {Boolean(actionMissingDescription) && <span>{actionMissingDescription} vaga{actionMissingDescription === 1 ? "" : "s"} sem descrição íntegra {actionMissingDescription === 1 ? "está bloqueada" : "estão bloqueadas"} até uma nova coleta e não {actionMissingDescription === 1 ? "entra" : "entram"} na classificação.</span>}
                  {actionCandidateCount === 0 && Boolean(actionCandidateTotal) && !reprocess && actionMissingDescription !== actionCandidateTotal && <span>Marque “Incluir vagas já triadas” para reavaliar as vagas desse recorte.</span>}
                ...
                <div className="triage-action-steps">
                  ...
                  <article className={`triage-action-step ${manualIsActive ? "waiting" : ""}`}>
                    <span>2</span><div><b>Enviar candidaturas</b><small>Use após a etapa 1 concluir. Envia automaticamente somente vagas ✅ com e-mail válido.</small></div>
                    <button className="triage-queue-button" disabled={queueingDrafts || runningPilot || manualIsActive || !actionSourceId} onClick={queueDrafts} title={manualIsActive ? "Aguarde a triagem concluir antes de enviar candidaturas." : !actionSourceId ? "Selecione uma fonte para enviar as candidaturas elegíveis do recorte." : undefined}>{queueingDrafts ? "Enviando…" : "Enviar"}</button>
                  </article>
                ...
                {csvImportOpen && <section className="triage-csv-import triage-ai-prompt" aria-label="Reimportar análise externa em CSV">
                  ...
                  {csvImportResult && <div className="triage-csv-import-result" aria-live="polite">
                    <span><b>{csvImportResult.applied}</b> veredito(s) substituído(s){csvImportResult.emailsSent ? `, ${csvImportResult.emailsSent} candidatura(s) enviada(s)` : csvImportResult.draftsCreated ? `, ${csvImportResult.draftsCreated} rascunho(s) criado(s), com envio não confirmado` : ""}.</span>
              ...
              <small>Acompanhe o resultado no cartão “Último lote manual”, logo abaixo. Toda decisão ✅ com e-mail válido cria automaticamente um rascunho com currículo e assinatura. O envio só acontece após sua confirmação no botão “Enviar”. Vereditos 🟡 continuam aguardando revisão.</small>
              </div>
            </details>
          </div>
        </div>
        ...
        {queueUsage && <section className={`triage-observability ${queueUsage.status}`} aria-label="Observabilidade da coleta e da triagem" aria-live="polite">
          ...
          <div className="triage-observability-grid">
            ...
            <article>
              <small>Proteção de leituras D1</small>
              <strong>Ativa</strong>
              <p>Histórico sem polling · progresso compacto em 30 s · cron em 15 min · varreduras globais no máximo a cada hora.</p>
            </article>
          </div>
        ...
        <section className="triage-automation" aria-label="Automação diária e saúde operacional">
          <div className="triage-automation-intro">
            ...
            <small>Triagem por regras para as vagas recebidas no dia; cada aprovação segura com e-mail válido aciona o rascunho imediatamente.</small>
          </div>
          ...
          <section id="triage-history" className="triage-history" aria-label="Histórico persistido da nova triagem">
            ...
            <div className="triage-list">
              ...
              <div className="triage-run-settings triage-table-filters">
                ...
                <button type="button" className="triage-clear-filters" onClick={() => clearHistoryFilters(situationFilter)}>{situationFilter === "pending" ? "Limpar outros filtros" : "Limpar filtros"}</button>
              </div>
