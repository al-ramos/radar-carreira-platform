...
export default function TriageReport({ close, sourceId, sourceLabel, sourceOptions = [], areaOptions = [], channelOptions = [], initialArea = "all", initialChannel = "all", homePeriod = "24", onOpenInRadar }: { close: () => void; sourceId?: string; sourceLabel?: string; sourceOptions?: FilterOption[]; areaOptions?: FilterOption[]; channelOptions?: FilterOption[]; initialArea?: string; initialChannel?: string; homePeriod?: "24" | "72" | "168" | "all"; onOpenInRadar: (code: string) => void }) {
        ...
        {(
          <section id="triage-history" className="triage-history" aria-label="Histórico persistido da nova triagem">
            ...
            <div className="triage-list">
              ...
              <div className="triage-table-wrap"><table className="triage-table"><thead><tr><th className="triage-select-column"><input aria-label="Selecionar todas as vagas visíveis" type="checkbox" checked={allVisibleSelected} onChange={toggleVisibleHistory} /></th><th><button onClick={() => sortHistory("verdict")}>Veredito</button></th><th><button onClick={() => sortHistory("title")}>Vaga</button></th><th><button onClick={() => sortHistory("company")}>Empresa</button></th><th>Fonte / código</th><th>Local e modalidade</th><th>Publicada / recebida</th><th>Contato</th><th><button onClick={() => sortHistory("draft")}>Rascunho</button></th><th>Envio</th><th><button onClick={() => sortHistory("processedAt")}>Analisada</button></th><th>Ações</th></tr></thead><tbody>{visibleHistory.length ? visibleHistory.map((item) => { const draftBlocker = draftActionBlocker(item); const openInRadar = () => { if (item.externalId) onOpenInRadar(item.externalId); }; return <tr key={item.id} className={`${rowClass[item.verdict] ?? "backlog"}${item.externalId ? " triage-row-clickable" : ""}`} onClick={openInRadar} tabIndex={item.externalId ? 0 : undefined} onKeyDown={(e) => { if (item.externalId && (e.key === "Enter" || e.key === " ")) { e.preventDefault(); openInRadar(); } }} title={item.externalId ? `Abrir vaga ${item.externalId} no Radar` : undefined}><td className="triage-select-column" onClick={(e) => e.stopPropagation()}><input aria-label={`Selecionar ${item.title}`} type="checkbox" checked={selectedHistoryJobIds.includes(item.jobId)} onChange={() => toggleHistoryJob(item.jobId)} /></td><td className="triage-verdict">{item.verdict}<small>{item.source === "ai" ? "IA" : item.source === "legacy" ? "Radar" : "Regras"}</small></td><td onClick={(e) => e.stopPropagation()}><a href={item.url} target="_blank" rel="noreferrer"><b>{item.title}</b></a><span>{item.label}{item.blocker ? ` · ${item.blocker}` : ""}</span>{item.source === "ai" && <details><summary>Evidências</summary><pre>{item.rows}</pre></details>}</td><td>{item.company}</td><td>{item.jobSource ? sourceName(item.jobSource) : "Não informada"}<small>{item.externalId ? `Código ${item.externalId}` : "Sem código"}</small></td><td>{item.workMode ?? "—"}<small>{item.location ?? "Local não informado"}</small></td><td>{item.sourcePublishedAt ? date(item.sourcePublishedAt) : "Não informada"}<small>Recebida: {date(item.receivedAt)}</small></td><td>{item.hasValidContactEmail ? item.contactEmail : "Manual / sem e-mail"}</td><td>{item.draftStatus === "sent" ? "Rascunho usado" : item.draftStatus === "drafted" ? "Pronto" : item.draftStatus === "pending" ? "Na fila" : item.draftStatus === "failed" ? "Falhou" : "—"}</td><td>{item.sentAt ? <><b>Enviado</b><small>{date(item.sentAt)}</small></> : item.draftStatus === "drafted" ? "Ainda não enviado" : "—"}</td><td>{date(item.processedAt)}</td><td onClick={(e) => e.stopPropagation()}><div className="triage-row-actions"><button type="button" onClick={() => void openAiPrompt([item.jobId])} disabled={aiReviewLoading} title="Abre a consulta individual no painel acima; não altera o veredito nem cria rascunho">Consultar IA</button><button type="button" onClick={() => void queueDrafts([item.jobId])} disabled={queueingDrafts || Boolean(draftBlocker)} title={draftBlocker ?? "Revalida a vaga e prepara somente a fila; não envia e-mail."}>Rascunho</button>{draftBlocker && <small>{draftBlocker}</small>}</div></td></tr>; }) : <tr><td className="triage-table-empty" colSpan={12}>Nenhuma vaga corresponde aos filtros selecionados.</td></tr>}</tbody></table></div>
            </div>
          </section>
        )}
