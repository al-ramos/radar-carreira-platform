...
export default function TriageReport({ open = true, close, openJobInRadar, sourceId, sourceLabel, sourceOptions = [], areaOptions = [], channelOptions = [], initialArea = "all", initialChannel = "all", homePeriod = "24", highlightBatchId }: { open?: boolean; close: () => void; openJobInRadar: (job: Pick<HistoryItem, "jobId" | "externalId" | "jobSource">) => void; sourceId?: string; sourceLabel?: string; sourceOptions?: FilterOption[]; areaOptions?: FilterOption[]; channelOptions?: FilterOption[]; initialArea?: string; initialChannel?: string; homePeriod?: "24" | "72" | "168" | "all"; highlightBatchId?: string }) {
  const [message, setMessage] = useState("Carregando avaliações…"),
    ...
    [disqualifyingJobId, setDisqualifyingJobId] = useState<string | null>(null),
  ...
  const disqualifyJob = async (item: HistoryItem) => {
    if (!window.confirm(`Desclassificar a vaga “${item.title}”? Ela será marcada como não aderente e nenhum rascunho pendente será criado.`)) return;
    setDisqualifyingJobId(item.jobId);
    setMessage(`Desclassificando ${item.title}…`);
    try {
      ...
      const result = await readJsonResponse<{ error?: string; cancelledPendingDraft?: boolean }>(response, "A desclassificação da vaga");
      if (!response.ok) throw new Error(result.error ?? "Não foi possível desclassificar a vaga.");
      setSelectedHistoryJobIds((current) => current.filter((id) => id !== item.jobId));
      setMessage(`${item.title} foi desclassificada. ${result.cancelledPendingDraft ? "Qualquer rascunho ainda na fila foi cancelado." : "Nenhum rascunho pendente precisou ser cancelado."}`);
      await loadHistory();
    } catch (error) {
      setMessage(error instanceof Error ? error.message : "Não foi possível desclassificar a vaga.");
    } finally {
