...
const isApinfoJob = (item: Pick<HistoryItem, "jobSource" | "url">) => item.jobSource === "apinfo-extension" || /apinfo\.com/i.test(item.url);
...
export default function TriageReport({ close, openJobInRadar, sourceId, sourceLabel, sourceOptions = [], areaOptions = [], channelOptions = [], initialArea = "all", initialChannel = "all", homePeriod = "24", highlightBatchId }: { close: () => void; openJobInRadar: (job: Pick<HistoryItem, "jobId" | "externalId" | "jobSource">) => void; sourceId?: string; sourceLabel?: string; sourceOptions?: FilterOption[]; areaOptions?: FilterOption[]; channelOptions?: FilterOption[]; initialArea?: string; initialChannel?: string; homePeriod?: "24" | "72" | "168" | "all"; highlightBatchId?: string }) {
  const [message, setMessage] = useState("Carregando avaliações…"),
    ...
    [applicationFilter, setApplicationFilter] = useState("all"),
    ...
    [sortKey, setSortKey] = useState<"processedAt" | "company" | "title" | "description" | "verdict" | "draft" | "applicationActivityAt">("processedAt"),
    ...
    [csvImportSucceeded, setCsvImportSucceeded] = useState(false),
  ...
  const applicationState = (item: HistoryItem) => isApinfoJob(item)
    ? item.sentAt ? "sent" : item.draftStatus === "drafted" || item.draftStatus === "pending" ? "generated" : "none"
    : item.applicationStatus ?? "none";
  const applicationActivityAt = (item: HistoryItem) => isApinfoJob(item)
    ? item.sentAt ?? item.lastSentCheckAt ?? item.draftUpdatedAt ?? ""
    : item.applicationRespondedAt ?? item.applicationSentAt ?? item.applicationGeneratedAt ?? "";
  const scopedHistory = currentAssessments.filter((item) => (situationFilter === "all" || (situationFilter === "pending" ? isPending(item) : !isPending(item))) && (verdictFilter === "all" || item.verdict === verdictFilter) && (sourceFilter === "all" || item.source === sourceFilter) && (jobSourceFilter === "all" || item.jobSource === jobSourceFilter) && (!codeFilter.trim() || (item.externalId ?? "").toLowerCase().includes(codeFilter.trim().toLowerCase())) && (!descriptionFilter.trim() || item.description.toLowerCase().includes(descriptionFilter.trim().toLowerCase())) && (applicationFilter === "all" || applicationState(item) === applicationFilter) && (!publishedDateFilter || dayKey(item.sourcePublishedAt) === publishedDateFilter) && (!receivedDateFilter || dayKey(item.receivedAt) === receivedDateFilter) && (!analysedDateFilter || dayKey(item.processedAt) === analysedDateFilter));
  ...
  const orderedHistory = [...filteredHistory].sort((a, b) => {
    const fields = { processedAt: [a.processedAt, b.processedAt], company: [a.company, b.company], title: [a.title, b.title], description: [a.description, b.description], verdict: [a.verdict, b.verdict], draft: [a.draftStatus ?? "", b.draftStatus ?? ""], applicationActivityAt: [applicationActivityAt(a), applicationActivityAt(b)] } as const;
  ...
  const submitCsvImport = async () => {
    ...
    try {
      ...
      const result = await readJsonResponse<{ applied?: number; draftsQueued?: number; notFound?: string[]; ambiguous?: string[]; rejected?: Array<{ line: number; reason: string }>; error?: string; duplicate?: boolean }>(response, "A reimportação da análise");
      if (!response.ok) throw new Error(result.error ?? "Não foi possível reimportar a análise.");
      if (result.duplicate) {
        setCsvImportSucceeded(true);
        setMessage("Este CSV já foi importado. Nenhum veredito ou rascunho foi reenviado.");
        return;
      }
            ...
            <details className="triage-actions">
              ...
              <div className="triage-run-panel">
                ...
                {csvImportOpen && <section className="triage-csv-import triage-ai-prompt" aria-label="Reimportar análise externa em CSV">
                  ...
                  <textarea aria-label="Conteúdo CSV da análise" value={csvImportText} onChange={(event) => { setCsvImportText(event.target.value); setCsvImportSucceeded(false); setCsvImportResult(null); }} placeholder={"codigo,status,descricao\n85981,🟡,Provável com ressalvas"} disabled={csvImportLoading} />
                  <div><button type="button" className="triage-queue-button" onClick={() => { setCsvImportOpen(false); setCsvImportText(""); setCsvImportSucceeded(false); setCsvImportResult(null); }} disabled={csvImportLoading}>Cancelar</button><button type="button" className="primary" onClick={() => void submitCsvImport()} disabled={csvImportLoading || csvImportSucceeded || !csvImportText.trim()}>{csvImportLoading ? "Importando…" : csvImportSucceeded ? "Importação concluída" : "Substituir vereditos"}</button></div>
          ...
          <section id="triage-history" className="triage-history" aria-label="Histórico persistido da nova triagem">
            ...
            <div className="triage-list">
              <div className="triage-run-settings triage-table-filters">
                ...
                <label>Descrição<input type="search" placeholder="Filtrar descrição" value={descriptionFilter} onChange={(e) => { setDescriptionFilter(e.target.value); setHistoryPage(0); }} /></label>
                <label>Candidatura / envio<select value={applicationFilter} onChange={(e) => { setApplicationFilter(e.target.value); setHistoryPage(0); }}><option value="all">Todos</option><option value="none">Pendentes</option><option value="generated">Rascunho / preparada</option><option value="sent">Enviada</option><option value="responded">Resposta recebida</option></select></label>
                ...
                <button type="button" className="triage-clear-filters" onClick={() => { setSituationFilter("pending"); setVerdictFilter("all"); setSourceFilter("all"); setJobSourceFilter("apinfo-extension"); setCodeFilter(""); setDescriptionFilter(""); setApplicationFilter("all"); setDraftFilter("all"); setPublishedDateFilter(""); setReceivedDateFilter(""); setAnalysedDateFilter(""); setAdvancedFiltersOpen(false); setHistoryPage(0); }}>Fila pendente da APInfo</button>
              </div>
              ...
              <div className="triage-table-wrap"><table className="triage-table"><thead><tr><th className="triage-select-column"><input aria-label="Selecionar todas as vagas visíveis" type="checkbox" checked={allVisibleSelected} onChange={toggleVisibleHistory} /></th><th><button onClick={() => sortHistory("verdict")}>Veredito</button></th><th><button onClick={() => sortHistory("title")}>Vaga</button></th><th><button onClick={() => sortHistory("description")}>Descrição</button></th><th><button onClick={() => sortHistory("company")}>Empresa</button></th><th>Fonte / código</th><th>Local e modalidade</th><th>Publicada / recebida</th><th>Contato</th><th><button onClick={() => sortHistory("draft")}>Rascunho</button></th><th><button onClick={() => sortHistory("applicationActivityAt")}>Candidatura / envio</button></th><th><button onClick={() => sortHistory("processedAt")}>Analisada</button></th><th>Ações</th></tr></thead><tbody>{visibleHistory.length ? visibleHistory.map((item) => {
