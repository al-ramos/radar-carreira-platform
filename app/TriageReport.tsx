...
export default function TriageReport({ open = true, close, openJobInRadar, sourceId, sourceLabel, sourceOptions = [], areaOptions = [], channelOptions = [], initialArea = "all", initialChannel = "all", homePeriod = "24", highlightBatchId }: { open?: boolean; close: () => void; openJobInRadar: (job: Pick<HistoryItem, "jobId" | "externalId" | "jobSource">) => void; sourceId?: string; sourceLabel?: string; sourceOptions?: FilterOption[]; areaOptions?: FilterOption[]; channelOptions?: FilterOption[]; initialArea?: string; initialChannel?: string; homePeriod?: "24" | "72" | "168" | "all"; highlightBatchId?: string }) {
  ...
  const draftActionBlocker = (item: HistoryItem) => {
    if (item.draftStatus) return item.draftStatus === "sent" ? (item.gmailSentId ? "Enviado confirmado pelo Gmail" : "Envio informado manualmente") : item.draftStatus === "drafted" ? "Rascunho pronto" : item.draftStatus === "pending" ? null : item.draftError ?? "A criação imediata falhou; tente novamente após corrigir o conector Gmail.";
  ...
  const queueDrafts = async (jobIds?: string[]) => {
    ...
    try {
      ...
      let result = await readJsonResponse<{ error?: string; considered: number; queued: number; noValidContact: number; outdated: number; alreadyPresent: number; immediateDraft?: { requested: boolean; created?: number; sent?: number; reason?: string } }>(response, "O envio da candidatura");
      ...
      if (jobIds?.length === 1 && result.outdated > 0) {
        ...
        result = await readJsonResponse<{ error?: string; considered: number; queued: number; noValidContact: number; outdated: number; alreadyPresent: number; immediateDraft?: { requested: boolean; created?: number; sent?: number; reason?: string } }>(response, "O envio da candidatura");
      ...
      const immediateMessage = result.immediateDraft?.requested
        ? result.immediateDraft.sent ? ` ${result.immediateDraft.sent} candidatura(s) foi(ram) enviada(s) automaticamente pelo Gmail.` : result.immediateDraft.created ? ` ${result.immediateDraft.created} rascunho(s) foi(ram) criado(s), mas o envio ainda não foi confirmado.` : " O Gmail foi acionado; atualize a tela em instantes para confirmar o envio."
