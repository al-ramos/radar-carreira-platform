...
export async function POST(request:Request){
 ...
 const matches=recent.map(job=>{const match=scoreJob({title:job.title,description:job.description,stack:parse(job.stack),seniority:job.seniority,workMode:job.workMode,location:job.location,publishedAt:job.publishedAt},{masteredSkills:listFromStored(profile.masteredSkills),desiredAreas:listFromStored(profile.desiredAreas),avoidTerms:listFromStored(profile.avoidTerms),seniority:listFromStored(profile.seniority),preferredMode:listFromStored(profile.preferredMode)});return{...job,score:match.score,reasons:match.reasons}}).filter(job=>job.score>=preference.minScore).sort((a,b)=>b.score-a.score).slice(0,10);
