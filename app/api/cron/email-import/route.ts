...
export async function POST(request:Request){
  ...
  for(const email of payload.messages){const signal=applicationFromEmail(email);if(!signal)continue;const condition=signal.title&&signal.company?and(like(jobs.title,`%${signal.title}%`),like(jobs.company,`%${signal.company}%`)):signal.title?like(jobs.title,`%${signal.title}%`):signal.company?like(jobs.company,`%${signal.company}%`):null;if(!condition)continue;const matches=await db.select({id:jobs.id,title:jobs.title,company:jobs.company,externalId:jobs.externalId}).from(jobs).where(condition).orderBy(desc(jobs.updatedAt)).limit(2);if(matches.length!==1)continue;const matched=matches[0],jobId=matched.id,sentAt=new Date(email.date),existing=(await db.select().from(userJobStatus).where(and(eq(userJobStatus.userId,config.userId),eq(userJobStatus.jobId,jobId))).limit(1))[0],alreadySent=existing?.applicationStatus==="sent"||existing?.applicationStatus==="responded",applicationStatus=alreadySent?existing!.applicationStatus:"sent",stage=resolveAutomaticStage(existing?.stage,signal.stage);await db.insert(userJobStatus).values({userId:config.userId,jobId,stage,note:signal.detail,applicationStatus,generatedAt:existing?.generatedAt??sentAt,sentAt:existing?.sentAt??sentAt,respondedAt:existing?.respondedAt??null,updatedAt:sentAt}).onConflictDoUpdate({target:[userJobStatus.userId,userJobStatus.jobId],set:{stage,note:signal.detail,applicationStatus,generatedAt:existing?.generatedAt??sentAt,sentAt:existing?.sentAt??sentAt,respondedAt:existing?.respondedAt??null,updatedAt:sentAt}});await db.insert(jobEvents).values({jobId,type:signal.type,detail:signal.detail,occurredAt:sentAt});if(!alreadySent)await notifyDetectedApplication(db,{jobId,title:matched.title,company:matched.company,externalId:matched.externalId,evidence:signal.detail,detectedAt:sentAt}).catch(()=>undefined);events++}
