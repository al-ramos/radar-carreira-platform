...
export async function POST(request:Request){
  ...
  await db.insert(importRuns).values({id:runId,source:"Gmail/RadarVagas",status:"running",received:payload.messages.length,actorUserId:config.userId,startedAt:now});
  let inserted=0,updated=0,events=0;
  for(const job of imported){const fp=fingerprint(job),identity=job.externalId?or(eq(jobs.externalId,job.externalId),eq(jobs.fingerprint,fp)):eq(jobs.fingerprint,fp),existing=(await db.select({id:jobs.id}).from(jobs).where(identity).limit(1))[0],values={id:existing?.id??crypto.randomUUID(),fingerprint:fp,sourceId:null,externalId:job.externalId??null,company:job.company,title:job.title,seniority:null,workMode:job.workMode??null,location:job.location??null,stack:"[]",publishedAt:job.publishedAt?new Date(job.publishedAt):now,url:job.url,description:job.description??"",firstSeenAt:now,lastSeenAt:now,status:"active" as const,createdAt:now,updatedAt:now};if(existing){await db.update(jobs).set({fingerprint:fp,externalId:values.externalId,company:values.company,title:values.title,workMode:values.workMode,location:values.location,publishedAt:values.publishedAt,url:values.url,description:values.description,lastSeenAt:now,status:"active",updatedAt:now}).where(eq(jobs.id,existing.id));updated++}else{await db.insert(jobs).values(values);inserted++}}
