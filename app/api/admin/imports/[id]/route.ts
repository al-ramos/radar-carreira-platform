...
export async function GET(_request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const intake = typeof intakeSource.valid === "number" || typeof intakeSource.invalid === "number" || typeof intakeSource.rejectedProfile === "number"
    ? { valid: typeof intakeSource.valid === "number" ? intakeSource.valid : null, invalid: typeof intakeSource.invalid === "number" ? intakeSource.invalid : 0, invalidReasons: typeof intakeSource.invalidReasons === "object" && intakeSource.invalidReasons ? intakeSource.invalidReasons : {}, rejectedProfile: typeof intakeSource.rejectedProfile === "number" ? intakeSource.rejectedProfile : 0, rejectedJobs: Array.isArray(intakeSource.rejectedJobs) ? intakeSource.rejectedJobs.filter((job): job is { externalId?: string; title: string; company: string; reason: string } => Boolean(job) && typeof job === "object" && typeof (job as { title?: unknown }).title === "string" && typeof (job as { company?: unknown }).company === "string" && typeof (job as { reason?: unknown }).reason === "string").slice(0, 2000) : [], accepted: typeof intakeSource.accepted === "number" ? intakeSource.accepted : null, profileRule: typeof intakeSource.profileRule === "string" ? intakeSource.profileRule : null }
    : null;
