...
import { aiUsageEvents, jobs, profiles, triageAiReviewChunks, triageAiReviews, userJobAnalyses } from "../../../../db/schema";
...
export async function POST(request: Request) {
  ...
  const body = await request.json().catch(() => null) as Record<string, unknown> | null, sourceId = typeof body?.sourceId === "string" ? body.sourceId.trim() : "", homePeriod = typeof body?.homePeriod === "string" ? body.homePeriod : "24", roleArea = typeof body?.roleArea === "string" ? body.roleArea.trim() : "", ingestionChannel = typeof body?.ingestionChannel === "string" ? body.ingestionChannel.trim() : "", prompt = typeof body?.prompt === "string" ? body.prompt.trim().slice(0, 1200) : "", includeTriaged = body?.includeTriaged === true;
  ...
  if ((!sourceId || sourceId === "all") && !requestedJobIds?.length) return NextResponse.json({ error: "Selecione uma fonte ou vagas antes de pedir a análise." }, { status: 400 });
