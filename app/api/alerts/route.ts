...
export async function GET(){const u=await getChatGPTUser();if(!u)return NextResponse.json({error:"Autenticação necessária"},{status:401});const db=getDb(),profile=(await db.select().from(profiles).where(eq(profiles.userId,u.userId)).limit(1))[0];if(!profile)return NextResponse.json({alerts:[],unread:0,preferences:{enabled:true,minScore:80,frequency:"daily"}});
 ...
 const alerts=rows.map(j=>{const stack=parse(j.stack),match=scoreJob({title:j.title,description:j.description,stack,seniority:j.seniority,workMode:j.workMode,location:j.location,publishedAt:j.publishedAt},{masteredSkills:listFromStored(profile.masteredSkills),desiredAreas:listFromStored(profile.desiredAreas),avoidTerms:listFromStored(profile.avoidTerms),seniority:listFromStored(profile.seniority),preferredMode:listFromStored(profile.preferredMode)});return{id:j.id,title:j.title,company:j.company,url:j.url,publishedAt:j.publishedAt,score:match.score,reasons:match.reasons,read:seen.has(j.id)}}).filter(a=>a.score>=pref.minScore);
