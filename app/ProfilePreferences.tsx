...
export default function ProfilePreferences({ value, onChange, onSave, onClose, message, isAdmin, isOwner, aiStatus }: Props) {
  ...
  return <div className="modal-backdrop" onClick={onClose}>
    <section className="modal profile-choice-modal" onClick={event => event.stopPropagation()}>
      ...
      <ChoiceField label="Stack principal obrigatória" hint="Quando preenchida, a vaga precisa citar ao menos uma destas tecnologias ou atender a uma exceção." options={SKILL_OPTIONS} groups={SKILL_GROUPS} value={value.careerRules.coreStack} onChange={next => updateCareerRule("coreStack", next)} customPlaceholder="Adicionar tecnologia principal" />
