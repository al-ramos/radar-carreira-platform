  ...
  return <div className="modal-backdrop">
    <section className="modal profile-choice-modal" role="dialog" aria-modal="true" aria-labelledby="profile-preferences-title">
      ...
      <section className="career-profile-section" aria-labelledby="career-positioning-title">
        ...
        <label className="career-disclosure"><input type="checkbox" checked={value.careerRules.acceptOptionalRequirements} onChange={event => updateCareerRule("acceptOptionalRequirements", event.target.checked)} /><span><strong>Aceitar requisitos diferenciais</strong><small>Não rebaixa vagas por tecnologias descritas como desejáveis ou diferenciais; elas não passam a constar como competências dominadas.</small></span></label>
        <label className="career-disclosure"><input type="checkbox" checked={value.careerRules.acceptUnspecifiedContracts} onChange={event => updateCareerRule("acceptUnspecifiedContracts", event.target.checked)} /><span><strong>Aceitar contrato não informado</strong><small>Não rebaixa a vaga apenas porque o regime de contratação não foi divulgado.</small></span></label>
        <label className="career-disclosure"><input type="checkbox" checked={value.careerRules.acceptOnsiteWithinAcceptedRegions} onChange={event => updateCareerRule("acceptOnsiteWithinAcceptedRegions", event.target.checked)} /><span><strong>Aceitar presencial nas regiões escolhidas</strong><small>Considera presencial como compatível quando a localidade estiver entre as regiões aceitas.</small></span></label>
        <label className="career-disclosure"><input type="checkbox" checked={value.careerRules.filterImportsByCoreStack} onChange={event => updateCareerRule("filterImportsByCoreStack", event.target.checked)} /><span><strong>Restringir a importação à stack principal</strong><small>Quando desativado, o Radar importa as vagas e decide na triagem; quando ativado, descarta antes da triagem as vagas fora da stack principal.</small></span></label>
      ...
      <ChoiceField label="Competências dominadas" hint="Elas valem até 60 pontos: a primeira família compatível vale 35 e famílias adicionais aumentam a aderência." options={SKILL_OPTIONS} groups={SKILL_GROUPS} value={value.masteredSkills} onChange={next => update("masteredSkills", next)} customPlaceholder="Adicionar tecnologia" />
